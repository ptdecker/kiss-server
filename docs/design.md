...
2. **Router dispatch** — matches the method and path to the first registered handler. Exact routes
   take priority over prefix routes; first registration wins on ties. The router rejects dot-dot
...
- **Context**: Wraps the incoming `Request`, the outgoing `Response`, and optional `AuthClaims` for
  a single HTTP request and response cycle. Constructed per-request in `handle_connection`.
  Handlers read from `ctx.request` and write into `ctx.response` in place. `ctx.auth` carries the
...
- The first `ShortCircuit` stops the chain — later middleware does not run.
- `AuthMiddleware` (`src/server/auth.rs`) validates `X-Authenticated-User` and populates `ctx.auth`
  on success, or writes a 401 response and returns `ShortCircuit` on failure.
...
- If plugins are ever needed without a vhost config, the right fix is a minimal config file with
  only `[[plugin]]` blocks, not plugin flags on `--root`.
